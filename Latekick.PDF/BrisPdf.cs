...
namespace Latekick.PDF
{
    public class BrisPdf : BasePdf
    {
        ...
        #region Constructors
        public BrisPdf(string directoryname, string filename)
        {
        ...
        public bool CreateBRISPdf(XDocument xd)
        {
        ...
        /*private PdfPTable CreatePageHeader(string HorseList)
        {
            PdfPTable DocHeaderTable = new PdfPTable(2);
            DocHeaderTable.KeepTogether = true;
            DocHeaderTable.WidthPercentage = 100;

            Phrase phPageCourseDate = new Phrase(HorseList, largeboldfont);
            Phrase phPageHeaderRaceDetails = new Phrase("", largeboldfont);
        ...
        */
        ...
        private PdfPTable CreateHorseHeader(Latekick.BOL.Base.Entry CurrentEntry, Latekick.BOL.Base.Race CurrentRace)
        {
            ...
            Phrase phJName = new Phrase(Utility.ti.ToTitleCase(CurrentEntry.JockeyName.ToLower()).Replace("Ii", "II").Replace("IIi", "III") + (CurrentEntry.ApprenticeAllowance == null ? "" : " (" + CurrentEntry.ApprenticeAllowance + ")").ToString() + " (" + Math.Round((double)(100 * CurrentEntry.Jockey_Meet.Win / Math.Max(1, CurrentEntry.Jockey_Meet.Runs))) + "%)", smallbluefont);
            Phrase phTTitle = new Phrase(CurrentEntry.TrainerName.ToLower() == "" ? "" : " T: ", smallboldfont);
            Phrase phTName = new Phrase(Utility.ti.ToTitleCase(CurrentEntry.TrainerName.ToLower()).Replace("Ii", "II").Replace("IIi", "III") + " (" + Math.Round((double)(100*CurrentEntry.Trainer_Meet.Win / Math.Max(1,CurrentEntry.Trainer_Meet.Runs))) + "%)", smallbluefont);
            ...
            Phrase phATitle = new Phrase("Age: ", smallboldfont);
            Phrase phAName = new Phrase(CurrentEntry.ThisHorse.Age.ToString() + " " + CurrentEntry.ThisHorse.Gender + ".", smallbluefont);
            ...
            for (int i = 0; i < CurrentEntry.ThisHorse.PastPerformances.Length; i++)
            {
                ...
                for (int j = 1; j <= arrTop3.Length; j++)
                {
                    if (j > 1)
                        separator = "; ";
                    Phrase phTop3Horse;
                    if (CurrentRace.HorseNames.Contains(arrTop3[j - 1]))
                ...
                PdfPCell cPerfComment = new PdfPCell(new Phrase(CurrentEntry.ThisHorse.PastPerformances[i].PerformanceComment, smallfont));
                PdfPCell cEmpty = new PdfPCell(new Phrase(""));
        ...
        private PdfPCell CreateWorkoutsCell(Latekick.BOL.Base.Entry CurrentEntry)
        {
            ...
            for (int w = 0; w < Math.Min(6,CurrentEntry.ThisHorse.Workouts.Length); w++)
                strWorks += CurrentEntry.ThisHorse.Workouts[w].WorkDate.ToString("ddMMM") + " " + CurrentEntry.ThisHorse.Workouts[w].WorkCourse + " (" + CurrentEntry.ThisHorse.Workouts[w].WorksSurface + ") " + CurrentEntry.ThisHorse.Workouts[w].WorkDistance + "f " + CurrentEntry.ThisHorse.Workouts[w].WorkTime + " (rating " + CurrentEntry.ThisHorse.Workouts[w].WorkRating + ")" + "        ";
